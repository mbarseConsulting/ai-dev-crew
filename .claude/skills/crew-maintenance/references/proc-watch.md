# proc-watch — keep crew's rules from going stale

Loaded by `SKILL.md` under `-w`. Options: **Full** (default) — every target declared in `references/sources.md`; `--scoped <target>` — one techno or one reference, e.g. after a major release.
...
1. **Scope.** The targets are the files in `../crew/technos/` and `../crew/references/`. A target with no feed in `references/sources.md` cannot be watched: list it in the digest. General industry news is out of scope — the only findings that matter are those able to change an existing rule.
...
5. **Write the digest** to `docs/watch/YYYY-MM-DD.md` at the root of the library's repository: what was checked, what changed, what it impacts, every claim with its source link. End it with a **"sources silent this run"** line naming every feed that produced nothing, and a **"not watchable"** line naming every target without a feed.
...
- Never set a `Dernière passe de veille` date without a digest for that run
