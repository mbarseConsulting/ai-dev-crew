...
- Implement `equals`/`hashCode` on an entity so they are stable across the transient → persisted transition (see `## Pourquoi` below for the three valid strategies)
...
> une boîte (noms de classes, packages, stratégie d'id retenue) va dans le **fichier projet**, jamais ici.
>
