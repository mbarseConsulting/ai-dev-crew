...
- `persistence-craft` is populated as the reference for the format, and `java-craft` — previously four generic MUST lines — is brought to the same depth. `layering-craft` and `api-rest-craft` — the two bricks that motivated the transverse axes in the first place — are populated next. The remaining three (`kafka-craft`, `ws-craft`, `node-bff-craft`) ship as skeletons explicitly marked `À PEUPLER`. Marked, not silently thin.
