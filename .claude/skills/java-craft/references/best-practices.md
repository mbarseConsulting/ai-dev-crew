...
> dans `persistence-craft`. Ce qui relève des couches est dans `layering-craft`. Ce qui
> relève de la forme du contrat HTTP est dans `api-rest-craft`.
...
JPA exige un constructeur sans argument et des champs mutables — voir `persistence-craft`.)*
...
## 7. Tests — l'expression Spring du critère de `test-craft`

Le critère (quel comportement va dans quel étage) appartient à `test-craft`. Son expression
côté Spring :
