---
name: testfix
description: "Use when: (1) one or more tests are failing (KO) and need fixing, (2) deciding whether a failing test means the source is broken or the test is stale, (3) another skill or agent needs classify-and-fix rules for a failing test — this skill is their one home, referenced by name, never duplicated."
...
- Do NOT apply this skill to writing new tests for previously-untested behavior — this skill fixes already-written, already-failing tests; use `dev-loop` (which writes tests alongside new code) or `test-craft` (which designs missing test scenarios) instead
