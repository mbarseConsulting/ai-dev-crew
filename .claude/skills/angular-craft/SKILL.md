...
Same objective/subjective split `dev-loop` uses elsewhere in the crew: the rules above are mechanical enough to auto-fix in code touched this session. The following are broader guidelines — worth flagging, never worth silently rewriting working code over:
