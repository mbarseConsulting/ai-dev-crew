...
- Never change an existing contract (path, field, status code, error shape) without flagging it and stopping, per `crew-dev`'s API rule — an added optional field is compatible, a renamed or removed one is not
